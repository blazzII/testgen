<?php 
    ...
    //find all images with specified invId
    function getImagesByInvId($invId) {
        $db = acmeConnect();
        $sql = 'SELECT imgId, imgPath, imgName FROM images WHERE invId = :invId AND imgName LIKE \'%-tn.%\'';
        $stmt = $db->prepare($sql);
        $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
        $stmt->execute();
        $imageArray = $stmt->fetchAll(PDO::FETCH_ASSOC);
