...
switch ($action) {
    case 'upload':
        ...
        if($imageCheck){
            $message = '<p class="displayMessage">An image by that name already exists.</p>';
        } elseif (empty($invId) || empty($imgName)) {
            $message = '<p class="displayErrMessage">You must select a product and image file for the product.</p>';
        } else {
        ...
        if ($result) {
            $message = '<p class="displayMessage">The upload succeeded.</p>';
        } else {
            $message = '<p class="displayErrMessage">Sorry, the upload failed.</p>';
        }
        }
    ...
    case 'delete':
        ...
        $target = $image_dir_path . '/' . urlencode($filename);
        ...
        if ($remove) {
            $message = "<p class=\"displayMessage\">$filename was successfully deleted.</p>";
        } else {
            $message = "<p class=\"displayErrMessage\">$filename was NOT deleted.</p>";
        }
